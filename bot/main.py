...
class ScalpingBot:
    ...
    def _prepare_lstm_data(self) -> tuple[np.ndarray | None, np.ndarray | None]:
        ...
        try:
            ...
            # Create sequences
            X, y = [], []
            for i in range(len(data_scaled) - self.lstm_look_back):
                X.append(data_scaled[i:(i + self.lstm_look_back), :]) # Use both features
                y.append(scaled_prices[i + self.lstm_look_back, 0]) # Predict the next scaled price
    ...
    def _predict_price_lstm(self) -> Optional[float]:
        ...
        try:
            ...
            # Scale using the *same* fitted scalers - wrapped in try/except
            try:
                scaled_prices = self.scaler_price.transform(prices)
                scaled_volumes = self.scaler_volume.transform(volumes)
            except NotFittedError:
                logger.warning("Scaler not fitted yet. Skipping LSTM prediction.")
                return None
            ...
            # Log specific errors, e.g., if scaler wasn\'t fitted (already handled above)
    ...
    def _get_historical_data(self, interval="1m", limit=1000) -> pd.DataFrame:
        ...
        try:
            ...
            # Ensure limit doesn\'t exceed Binance capabilities for the timeframe
    ...
    def _make_decision(self, current_price: float, current_volume: float):
        ...
        if self.best_indicator == "ema":
            ...
            if rsi < 35 and combined_buy_signal: # Looser threshold
                ...
                should_sell = True
        elif self.best_indicator == "bbands":
            if current_price < lower_bb and combined_buy_signal:
                should_buy = True
            elif current_price > upper_bb and combined_sell_signal:
                should_sell = True
        elif self.best_indicator == "vwap":
            if current_price > vwap and ema9 > ema21 and combined_buy_signal: # VWAP + EMA confirmation
                should_buy = True
            elif current_price < vwap and ema9 < ema21 and combined_sell_signal:
                should_sell = True

        # --- Execute Trade --- 
        if should_buy:
            logger.info(f"BUY signal triggered based on {self.best_indicator}, prediction, and sentiment.")
            self._execute_trade(SIDE_BUY, current_price)
        elif should_sell:
            logger.info(f"SELL signal triggered based on {self.best_indicator}, prediction, and sentiment.")
            self._execute_trade(SIDE_SELL, current_price)
        else:
            logger.debug("No trade signal generated.")
    ...
    def _execute_trade(self, side: str, price: float):
        ...
        if not self.client:
            ...
            return
        if not self._check_order_limit():
            logger.warning("Cannot execute trade: Order limit reached.")
            return
        if not self._check_risk_limit():
             logger.warning("Cannot execute trade: Daily loss limit reached.")
             return
        ...
        try:
            ...
            if side == SIDE_BUY:
                ...
                sl_tp_side = SIDE_SELL # Sell to close long position
            else: # SIDE_SELL
                stop_loss_price = price * (1 + self.stop_loss_pct)
                take_profit_price = price * (1 - self.take_profit_pct)
                sl_tp_side = SIDE_BUY # Buy to close short position
            ...
            if price_filter and price_filter.get(\'tickSize\'):
                ...
                if \'.\' in tick_size_str:
                    ...
                    price_precision = 0 # Should not happen for USDT pairs
            else:
            ...
            logger.info(f"Attempting MARKET {side} order for {self.position_size} {self.symbol} at ~{price:.{price_precision}f}")
            # --- Place Market Order --- 
    ...
    def start_websocket(self):
        ...
        try:
            ...
            # --- Start User Data Stream (Basic Logging) --- 
            def handle_user_message(msg):
                logger.debug(f"User WS Message: {msg}")
                if msg.get(\'e\') == \'executionReport\':
                    # Basic logging of order updates
                    order_id = msg.get(\'i\')
                    status = msg.get(\'X\') # e.g., FILLED, CANCELED, EXPIRED
                    side = msg.get(\'S\')
                    price = msg.get(\'L\')
                    qty = msg.get(\'l\')
                    logger.info(f"User Stream: Order {order_id} update - Status: {status}, Side: {side}, Price: {price}, Qty: {qty}")
                    # TODO: Add logic here to find related trade in DB and update status/PnL
                elif msg.get(\'e\') == \'balanceUpdate\':
                     logger.info(f"User Stream: Balance Update - Asset: {msg.get(\'a\')}, Delta: {msg.get(\'d\')}")
                elif msg.get(\'e\') == \'outboundAccountPosition\':
                     # Log current balances periodically if needed
                     pass 
            
            self.ws_user_key = self.twm.start_user_socket(callback=handle_user_message)
            logger.info("User Data WebSocket started (basic logging enabled).")
            # --- End User Data Stream --- 
    ...
    def stop_websocket(self):
        ...
        try:
            ...
            # Stop specific streams first if keys exist
            if self.ws_kline_key: self.twm.stop_socket(self.ws_kline_key)
            if self.ws_user_key: self.twm.stop_socket(self.ws_user_key)
            self.twm.stop() # Stop the manager thread
            self.twm = None
            self.is_websocket_running = False
